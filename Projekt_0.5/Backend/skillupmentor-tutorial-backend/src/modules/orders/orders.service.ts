...
export class OrderService extends AbstractService{
    ...
    async chart(): Promise<{date:string;sum:string}[]>{
        ...
        SELECT to_data(cast(o.created_at as TEXT), '%Y-%m-%d) as data, sum(oi.price*oi.quantity) as sum FROM "order" o
        ...
        for (let index =0 ; index<apiData.lenght;[index++]){
            chartData.push({
                date:(apiData[index].data as Date).toISOString().split('T')[0],
